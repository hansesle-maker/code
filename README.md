...
### ⚠️ 바이낸스 "418 I'm a teapot" 오류가 뜬다면

전 종목을 짧은 시간에 스캔하면 바이낸스의 IP당 요청 가중치 한도를 넘겨
일시적으로 IP가 차단됩니다(429=경고, 418=차단). `tsi_signal/rate_limit.py`가
모든 바이낸스 REST 호출에 공용 슬라이딩 윈도우 속도 제한을 걸고, TSI/SMC
스캔이 서로 겹치지 않도록 전역 게이트로 직렬화해 이 문제를 예방합니다. 그래도
418이 뜨면 화면 상단 오류 배너에 해제 예상 시각이 표시되니, 그 시간까지
새로고침/수동 스캔 없이 기다리면 자동으로 정상화됩니다.
